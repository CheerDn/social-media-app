...
function HomeGuest() {
  ...
  function ourReducer(draft, action) {
    switch (action.type) {
      case "usernameImmediately":
        ...
        // valid only for alphanumeric characters
      ...
      case "usernameAfterDelay":
        if (draft.username.value.length < 3) {
          draft.username.hasErrors = true
          draft.username.message = "Username must at least 3 characters."
        }
      ...
      case "emailDuplicate":
        if (action.value) {
          ...
          draft.email.message = "That email is already be used."
        } else {
  ...
  // Username: send request and check uniqueness
  useEffect(() => {
    if (state.username.checkCount) {
      ...
      async function fetchResults() {
        try {
          ...
          dispatch({ type: "usernameDuplicate", value: response.data })
        } catch (e) {
  ...
  // Email: send request and check uniqueness
  useEffect(() => {
    if (state.email.checkCount) {
      ...
      async function fetchResults() {
        try {
          ...
          dispatch({ type: "emailDuplicate", value: response.data })
        } catch (e) {
          console.log("There was a problem or the request might be cancelled")
        }
      }
  ...
  function handleSubmit(e) {
    e.preventDefault()
    // to handle error message properly
